...
export async function updateSubtask(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!board_id || !column_id || !task_id || !subtask_id || typeof is_completed !== 'boolean') {
      const response: ApiResponse = {
        status: false,
        result: null,
        error: 'Invalid request data',
        message: 'All fields (board_id, column_id, task_id, subtask_id, is_completed) are required'
      };
    ...
    // Update the subtask
    const updated = DataService.updateSubtask(
      board_id,
      column_id,
      task_id,
      subtask_id,
