...
export class DataService {
  ...
  static getAllBoards(): { id: string; title: string }[] {
    return board_list.map(board => ({ id: board.id, title: board.title }));
  }

  // Get board by ID
  static getBoardById(boardId: string): Board | undefined {
    return board_list.find(board => board.id === boardId);
  }

  // Create new board
  static createBoard(title: string, columns: { title: string }[]): Board {
  ...
  // Update board
  static updateBoard(boardId: string, title: string, columns: any[]): Board | null {
    ...
    board_list = board_list.map(board => {
      if (board.id === boardId) {
        ...
        let updatedColumns = existingColumns.map(existingColumn => ({
          ...existingColumn,
          tasks: board.columns.find(column => column.id === existingColumn.id)?.tasks || []
        }));
  ...
  // Delete board
  static deleteBoard(boardId: string): boolean {
  ...
  // Delete column
  static deleteColumn(boardId: string, columnId: string): boolean {
    let deleted = false;
